...
sudo pamac install lua-language-server fish lazygit docker docker-compose docker-buildx bat clang python-pip valgrind polkit polkit-kde-agent polkit-qt5 lib32-polkit vls luarocks tree-sitter lua fd polkit-gnome
...
npm install -g eslint_d yarn vscode-langservers-extracted typescript typescript-language-server intelephense diagnostic-languageserver stylelint-lsp
...
cd ../ && stow manjaro_hyperland nvim alacritty backgrounds fonts ideavimrc neovim
